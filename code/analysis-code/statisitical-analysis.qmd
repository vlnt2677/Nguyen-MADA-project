...
data_clean$new_cases[is.na(data_clean$new_cases)] <- 0
data_clean$incidence_rate[is.na(data_clean$incidence_rate)] <- 0

data_clean <- na.omit(data_clean)

# In my intial analyses, there were an absurd amount of zeroes; this was most likely due to the data sets capturing days prior to the pandemic.
...
df_clean <- df_clean %>% 
  mutate(new_cases = as.integer(df_clean$new_cases))

class(df_clean$new_cases)
```
...
lm_preds<- predict(lm_fit, new_data = df_clean)

lm_metrics <- tibble(truth = df_clean$new_cases, predicted = lm_preds$.pred) %>%
...
lasso_preds<- predict(lasso_fit, new_data = df_clean)

lasso_metrics <- tibble(truth = df_clean$new_cases, predicted = lasso_preds$.pred) %>%
  metrics(truth = truth, estimate = predicted)
lasso_metrics
```
...
best_params <- select_best(forest_tune_res, metric = "rmse")  # Or "rsq"
best_params
